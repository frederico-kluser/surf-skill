...
Verify `surf-skill` is reachable:

```bash
surf-skill --version
```

If the command fails or `surf-skill` is not in PATH: **halt** and tell
the user:

> I need `surf-skill` to research the web for this plan.
> Install it once: `npm i -g surf-skill && surf-skill setup`
> Then ask me again.
...
Before opening the conversation, run **one** batched `surf-skill search`
...
1. **Run a targeted `surf-skill search` first** (cheap settings to keep
   cost down):
   ```bash
   surf-skill search "<specific decision> tradeoffs 2026" --max 2 --quiet
   ```
...
- (Optional) `surf-skill search "<verify topic>" --max 1` to spot-check
...
surf-plan doctor                     # verify surf-skill installed + key count
surf-plan --version
surf-plan --help

# Research (via surf-skill — the skill MUST be installed)
surf-skill search "Q1" "Q2" "Q3" --max 3 --quiet        # batch baseline
surf-skill search "specific decision" --max 2 --quiet    # targeted Phase 4
surf-skill search "X" --provider brave --mode fast       # cheap option
```
